...
This package expresses **TouchDesigner MCP** as a typed, task-adaptive agent harness. Its concrete capability remains: Control a running TouchDesigner instance through MCP to create operators, set parameters, wire networks, and execute Python. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in TouchDesigner MCP |
| --- | --- | --- |
| Memory | `entity_relationship_memory` | Retain visual brief plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `dependency_and_path_traversal` | Transform touchdesigner process into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through operator network while preserving stop conditions and user authority. |
